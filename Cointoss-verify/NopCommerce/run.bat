...
SET my_dir=C:\CCP\teamproject\Cointoss-verify\NopCommerce
SET STUB=stub
...
C:\Windows\Microsoft.NET\Framework64\v4.0.30319\MSBuild.exe Cointoss.csproj

call %POIROT_ROOT%\Poirot4.NET\BCT\BytecodeTranslator.exe /e:1 /lib:Stubs /ib /whole /heap:splitFields %file_name%.exe %STUB%.dll
::call C:\CCP\boogie\Poirot4.NET\BCT\BytecodeTranslator.exe /e:1 /lib:Stubs /ib /whole /heap:splitFields NopCommerce.exe stub.dll

call %POIROT_ROOT%\Corral\BctCleanup.exe %model_name%.bpl %clean_name%.bpl /main:PoirotMain.Main /include:poirot_stubs.bpl 

call %POIROT_ROOT%\Corral\corral.exe %clean_name%.bpl /recursionBound:2 /k:1 /main:PoirotMain.Main /trackAllVars /z3opt:MBQI=true /tryCTrace 

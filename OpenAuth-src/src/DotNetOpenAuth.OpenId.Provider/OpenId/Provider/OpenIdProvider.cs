...
namespace DotNetOpenAuth.OpenId.Provider {
	...
	public sealed class OpenIdProvider : IDisposable, IOpenIdHost {
		...
		public async Task<HttpResponseMessage> PrepareResponseAsync(IRequest request, CancellationToken cancellationToken = default(CancellationToken)) {
            ...
            if (response.Transport == MessageTransport.Indirect)
            {
                HttpResponseMessage result;

                foreach (IChannelBindingElement bindingElement in this.Channel.outgoingBindingElements)
                {
                    if (bindingElement.Protection == MessageProtections.TamperProtection)
                    {
                        /*
                ...
                ((IndirectSignedResponse)response).SessionID = ((AuthenticationRequest)request).SessionID;
                result = await this.Channel.PrepareResponseAsync_CCP(response, cancellationToken);
                var _signedMsg = response as ITamperResistantOpenIdMessage;
